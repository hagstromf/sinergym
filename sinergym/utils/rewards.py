...
class ShiftedExpReward(BaseReward):
    ...
    # TODO: Update description!!!!
        ...
        super(ShiftedExpReward, self).__init__(env)
    ...
    def _get_comfort(self, obs_dict: Dict[str, Any]) -> Tuple[float, List[float], bool]:
        ...
        comfort = 0.0
        for T in temps:
            comfort += self.shift - exp(max(self.temp_range[0] - T, 0) + max(T - self.temp_range[1], 0))
...
def test_ShiftedExpReward():
    ...
    env = gym.make(env_name, reward=ShiftedExpReward, reward_kwargs=reward_kwargs)
 
    rew_func = env.reward_fn
    obs_dict = {'month': 1, 'day': 1, 'year':1991, 'Zone Air Temperature(West Zone)': 0, 'Zone Air Temperature(East Zone)': 0}
    test_obs = [[23.5, 23.5], [21.0, 21.0], [27.0, 27.0]]
    test_comforts = []
    test_violations = []
    ...
    assert test_comforts[0] == approx(2.0)
    assert test_comforts[1] == approx(-1.436, 0.005)
    assert test_comforts[2] == approx(-10.778, 0.005)
